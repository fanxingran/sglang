...
# src/kernel_packs/gfx950/glm52/sparse_paged_mla/sparse_paged_mla_tp8_m1.py
import torch
import triton
...
@gluon.jit
def _value_fragment(
    shared, group, WIDTH: gl.constexpr, LAYOUT: gl.constexpr, M: gl.constexpr
):
    if M == 2:
        gl.static_assert(WIDTH == 128)
        if group == 0:
            selected = shared.slice(0, 128, 1)
        elif group == 1:
            selected = shared.slice(128, 128, 1)
        elif group == 2:
            selected = shared.slice(256, 128, 1)
        else:
            selected = shared.slice(384, 128, 1)
        value = selected.load(LAYOUT)
    elif WIDTH == 512:
...
def _partial(
    ...
    VALID_STYLE: gl.constexpr = 0,
    PAIRED_COPY: gl.constexpr = False,
):
    ...
    if VALID_STYLE == 1 and ASYNC:
        score_tokens = part * BLOCK + gl.arange(
            0, BLOCK, gl.SliceLayout(0, compact_layout)
        )
        if SELECTED % BLOCK == 0:
            score_slots = gl.load(S + row * S0 + score_tokens).to(gl.int64)
            mask = score_slots >= 0
        else:
            score_slots = gl.load(
                S + row * S0 + score_tokens, score_tokens < SELECTED, -1
            ).to(gl.int64)
            mask = (score_tokens < SELECTED) & (score_slots >= 0)
    if VALID_STYLE == 0 or not ASYNC:
        tokens = part * BLOCK + gl.arange(0, BLOCK, gl.SliceLayout(1, cache_layout))
        if SELECTED % BLOCK == 0:
    ...
    if ASYNC:
        if BLOCK == 128:
            ...
            gl.static_assert(BLOCK == 64 and WARPS == 4)
            copy_layout: gl.constexpr = gl.DistributedLinearLayout(
        ...
        copy_tokens = part * BLOCK + gl.arange(0, BLOCK, gl.SliceLayout(1, copy_layout))
        if SELECTED % BLOCK == 0:
            copy_slots = gl.load(S + row * S0 + copy_tokens).to(gl.int64)
            copy_active = copy_slots >= 0
        else:
            copy_slots = gl.load(
                S + row * S0 + copy_tokens, copy_tokens < SELECTED, -1
            ).to(gl.int64)
            copy_active = (copy_tokens < SELECTED) & (copy_slots >= 0)
        copy_dims = gl.arange(0, 512, gl.SliceLayout(0, copy_layout))
        offsets = copy_slots[:, None] * K0 + copy_dims[None, :]
        gl.amd.cdna4.async_copy.buffer_load_to_shared(
    ...
    if PV_WIDTH < WIDTH:
        ...
        value = _value_fragment(latent_shared, value_group, WIDTH, vb_layout, M)
        products = gl.amd.cdna4.mfma(
