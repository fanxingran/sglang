...
# src/kernel_packs/gfx950/glm52/sparse_paged_mla/sparse_paged_mla_tp4_m2.py
import torch
import triton
...
def _gather_words(
    ...
    if HARDWARE_ZERO:
        ...
        base = gl.where(valid, word_slots.to(gl.int32) * (K0 // 4), 1 << 29)
        base = gl.multiple_of(base, 4)
        words = gl.amd.cdna4.buffer_load(
            KV.to(gl.pointer_type(gl.uint32)), base[None, :] + START // 4 + d[:, None]
        )
    else:
...
def _gather_bytes(
    ...
    layout: gl.constexpr = gl.BlockedLayout([16, 1], [4, 16], [1, WAVES], [0, 1])
    base = gl.where(active, slots.to(gl.int32) * K0, -2147483648)
    base = gl.multiple_of(base, 16)
    d = gl.arange(0, WIDTH, gl.SliceLayout(1, layout))
    return gl.amd.cdna4.buffer_load(KV, base[None, :] + START + d[:, None])
...
def _attention_partials(
    ...
    HARDWARE_ZERO: gl.constexpr,
    BYTE_GATHER: gl.constexpr,
    DIRECT_OUTPUT: gl.constexpr,
    ASYNC_QUERY: gl.constexpr,
    QK_WIDTH: gl.constexpr,
):
    pid_split_value = gl.program_id(2)
    if ROWS <= 2:
        pid_split_value = pid_split_value.to(gl.uint32)
    row = 0 if ROWS == 1 else gl.program_id(0)
    if ROWS == 2:
        row = row.to(gl.uint32)
    head_start = 0 if HEADS <= BLOCK_H else gl.program_id(1) * BLOCK_H
    if ROWS == 2:
        head_start = head_start.to(gl.uint32)
    head_tile = head_start // 16
    if ROWS > 4 and ROWS <= 8:
        split = pid_split_value % SPLITS
        value_start = pid_split_value // SPLITS * BLOCK_V
    else:
        split = pid_split_value // (512 // BLOCK_V)
        value_start = pid_split_value % (512 // BLOCK_V) * BLOCK_V
    ...
    query_row = row
    if Q0 >= 2**31 or Q1 >= 2**31 or (ROWS - 1) * Q0 + (HEADS - 1) * Q1 + 576 >= 2**31:
        query_row = row.to(gl.int64)
        hq = hq.to(gl.int64)
    ...
    position_valid = (SELECTED > 0) & (SELECTED % BLOCK_S == 0) | (positions < SELECTED)
    ...
    buffer_query: gl.constexpr = (
        (BYTE_GATHER or ROWS == 2)
        and Q0 >= 0
        and (Q0 < 2**30)
        and (Q1 >= 0)
        and (Q1 < 2**30)
        and ((ROWS - 1) * Q0 + (HEADS - 1) * Q1 + 576 < 2**30 - 1)
    )
    if ASYNC_QUERY:
        ...
        q = gl.load(
            Q + query_row * Q0 + hq[:, None] * Q1 + dq[None, :],
            (HEADS % BLOCK_H == 0) | (hq[:, None] < HEADS),
            0.0,
        )
    if ROWS > 4 and ROWS <= 8:
    ...
    if not (ROWS > 2 and ROWS <= 8):
        if PACKED_GATHER:
            key = _gather_words(
                KV, slots, active, K0, 512, 0, WAVES, True, HARDWARE_ZERO
            )
        else:
    ...
    if buffer_query:
        ...
        q_rope = gl.load(
            Q + query_row * Q0 + rope_heads[:, None] * Q1 + 512 + qr[None, :],
            (HEADS % BLOCK_H == 0) | (rope_heads[:, None] < HEADS),
            0.0,
        )
    if ROWS > 4 and ROWS <= 8:
    ...
    if ROWS > 2 and ROWS <= 8:
        if PACKED_GATHER:
            ...
            key = _gather_bytes(KV, slots, active, K0, 512, 0, WAVES)
        else:
            key = gl.load(KV + slots[None, :] * K0 + dk[:, None], active[None, :], 0.0)
    if PACKED_GATHER:
        key_rope = _gather_words(
            KV, slots, active, K0, 64, 512, WAVES, False, HARDWARE_ZERO
        )
    else:
        key_rope = gl.load(
            KV + slots[None, :] * K0 + 512 + kr[:, None], active[None, :], 0.0
        )
    if not ASYNC_QUERY and (not (ROWS > 4 and ROWS <= 8)):
        q_buffer = gl.allocate_shared_memory(q.dtype, q.shape, latent_shared, q)
    ...
    if QK_WIDTH > 128:
        gl.static_assert(BLOCK_S == 64)
        full_key_layout: gl.constexpr = gl.SharedLinearLayout(
            [
                [1, 0],
                [2, 0],
                [4, 0],
        ...
        full_key = key_buffer._reinterpret(shape=[512, BLOCK_S], layout=full_key_layout)
        for offset_k in gl.static_range(0, 512, QK_WIDTH):
            q_operand = q_buffer.slice(offset_k, QK_WIDTH, 1).load(dot_a)
            key_operand = (
                full_key.slice(offset_k, QK_WIDTH, 0).load(dot_b).to(gl.bfloat16)
            )
    ...
    if DIRECT_OUTPUT and BLOCK_S == 128 and (ROWS <= 4):
        denominator_buffer = gl.allocate_shared_memory(
            gl.float32, [BLOCK_H], gl.SwizzledSharedLayout(1, 1, 1, [0]), denominator
        )
    ...
    for offset in gl.static_range(0, BLOCK_V, PV_WIDTH):
        if ROWS > 4 and ROWS <= 8 and (offset == 0):
            values = prefetched_raw_values.to(gl.bfloat16)
        elif ROWS > 8 and offset == 0:
            values = first_values
        else:
            panel_index = ((value_start + offset) // PV_WIDTH).to(gl.int32)
            values = key_buffer.index(panel_index).load(pv_b).to(gl.bfloat16)
        ...
        if DIRECT_OUTPUT:
            if BLOCK_S == 128 and ROWS <= 4:
                pv_denominator = denominator_buffer.load(gl.SliceLayout(1, pv_mma))
            else:
                pv_denominator = gl.convert_layout(
                    denominator, gl.SliceLayout(1, pv_mma)
                )
            if ROWS > 1 and (ROWS <= 4 or ROWS > 8):
                inverse_denominator = 1.0 / gl.where(
                    pv_denominator > 0, pv_denominator, 1.0
                )
                result = numerator * inverse_denominator[:, None]
            else:
                result = numerator / gl.where(
                    pv_denominator[:, None] > 0, pv_denominator[:, None], 1.0
                )
            gl.store(
                Partial + (row * HEADS + hm[:, None]) * 512 + vm[None, :],
                result.to(gl.bfloat16),
                (HEADS % BLOCK_H == 0) | (hm[:, None] < HEADS),
            )
        else:
            if INTERLEAVED:
                ...
                partial_offset = (
                    (pid_split_value % 4 * SPLITS + split) * HEADS + hm[:, None]
                ) * 144 + local_v[None, :]
            if BUFFER_PARTIALS:
    ...
    if not DIRECT_OUTPUT and value_start == 0:
...
def _merge_partials(
    ...
    if INTERLEAVED:
        ...
        if VALUE_MAJOR and (WIDTH == 8 or SKIP_EMPTY):
    ...
    BUFFER_OUTPUT: gl.constexpr = INTERLEAVED and (
        not SPLIT_MAJOR_STATS
        and (WIDTH == 8 or (VALUE_MAJOR and SKIP_EMPTY))
        or (SPLIT_MAJOR_STATS and MERGE_H == 16)
    )
...
def sparse_paged_mla(
    ...
    *,
    ...
    cache_extent = (kv_cache.shape[0] - 1) * cache_stride + 576
    bounded_cache = (
        kv_cache.shape[0] > 0
        and 0 < cache_stride < 2**31
        and (cache_extent < 2**31 - 1)
    )
    aligned_cache = cache_stride % 16 == 0 and kv_cache.storage_offset() % 16 == 0
    packed_gather = (
        (m <= 4 or m > 8)
        and bounded_cache
        and (cache_stride % 4 == 0)
        and (kv_cache.storage_offset() % 4 == 0)
    )
    hardware_zero = m <= 4 and bounded_cache and aligned_cache
    byte_gather = 4 < m <= 8 and bounded_cache and aligned_cache
    async_query = (
        m <= 2
        and query.storage_offset() % 8 == 0
        and (query.stride(1) % 8 == 0)
        and (m == 1 or query.stride(0) % 8 == 0)
        and (0 <= query.stride(0) < 2**30)
        and (0 <= query.stride(1) < 2**30)
        and (
            (m - 1) * query.stride(0) + (heads - 1) * query.stride(1) + 512 < 2**30 - 1
        )
    )
    qk_width = 256 if m == 1 and selected <= 64 else 512 if m <= 4 else 128
    interleaved = m > 1
    ...
    if selected <= 64:
        block_s = 64
    elif selected <= 128 and m <= 4:
        block_s, block_v, block_h, waves = (128, 128, 16, 8)
        query_lanes, query_waves, qk_width = (16, 2, 128)
    value_gap = 16 if m == 2 else 0
    record_stride = 8224 if m > 8 else 8192
    splits = max(1, triton.cdiv(selected, block_s))
    ...
    direct_output = selected <= block_s
    if direct_output:
        partial, stats = (output, output)
    elif m > 8 or m in (1, 2, 4):
        partial_elements = 1
        for size in partial_shape:
            partial_elements *= size
        stats_elements = m * stats_heads * splits * 2
        arena_gap = 32 if m == 2 else 0
        arena = torch.empty(
            (partial_elements + stats_elements + arena_gap,),
            dtype=torch.float32,
            device=query.device,
        )
        if m == 2:
            stats = arena[:stats_elements].view(stats_shape)
            partial = arena[stats_elements + arena_gap :].view(partial_shape)
        else:
