...
# src/kernel_packs/gfx950/glm52/sparse_paged_mla/sparse_paged_mla_tp8_m4.py
import torch
import triton
...
def _partial(
    ...
    tokens = part * BLOCK + gl.arange(0, BLOCK, validity_layout)
    if SELECTED % BLOCK == 0:
        slots = gl.load(S + row * S0 + tokens).to(gl.int64)
        active = slots >= 0
    else:
    ...
    if PV_WIDTH < WIDTH:
        ...
        value = _value_fragment(latent_shared, value_group, WIDTH, vb_layout, M == 2)
        products = gl.amd.cdna4.mfma(
