...
# src/kernel_packs/gfx950/glm52/sparse_paged_mla/sparse_paged_mla_tp4_m1024.py
import torch
import triton
...
def _step_attention(
    ...
    _set_priority(0 if rows == 1024 else 1)
...
def _attention_mfma(
    ...
    if rows == 1024 or rows == 2048:
        cache_layout: gl.constexpr = gl.BlockedLayout([1, 16], [32, 2], [2, 2], [1, 0])
    else:
        cache_layout: gl.constexpr = gl.BlockedLayout([1, 16], [32, 2], [4, 1], [1, 0])
    ...
    EARLY_EMPTY: gl.constexpr = rows == 1024 and cache_extent <= 2048
    if EARLY_EMPTY:
        proof_bits = gl.where(
            scan_slots >= 0,
    ...
    if has_values:
        ...
        if rows == 1024:
            ...
            resident_queries = ()
            for part in gl.static_range(4):
                resident_queries += (query_smem.slice(part * 128, 128, 1).load(lhs),)
            resident_queries += (query_smem.slice(512, 64, 1).load(lhs),)
        else:
            ...
            for part in gl.static_range(4):
                resident_queries += (
                    query_latent_smem.slice(part * 128, 128, 1).load(lhs),
                )
        ...
        probability_low_smem = gl.allocate_shared_memory(
            gl.float8e4nv,
            (16, 128),
            gl.SwizzledSharedLayout(16, 1, 8, [1, 0])
            if rows == 1024 or rows == 2048
            else gl.SwizzledSharedLayout(8, 1, 16, [1, 0]),
        )
        if not EARLY_EMPTY:
            proof_bits = gl.where(
                scan_slots >= 0,
            ...
            for dense_tile in range(selected_count // 128):
                if selected_count > 128:
                    ...
                    next_slots = gl.full(
                        (128,),
                        -1,
                        selected.dtype.element_ty,
                        gl.SliceLayout(1, cache_layout),
                    )
        ...
        empty_layout: gl.constexpr = gl.BlockedLayout([1, 8], [4, 16], [4, 1], [1, 0])
        empty_heads = gl.arange(0, 16, gl.SliceLayout(1, empty_layout))
        empty_dims = gl.arange(0, 512, gl.SliceLayout(0, empty_layout))
        zeros = gl.full((16, 512), 0, gl.bfloat16, empty_layout)
        gl.store(
            output + (row * heads + empty_heads[:, None]) * 512 + empty_dims[None, :],
            zeros,
            cache_modifier=".cs",
        )
